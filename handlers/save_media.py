...
async def save_batch_media_in_channel(bot: Client, editable: Message, message_ids: list):
    try:
        ...
        share_link = f"get_short(https://t.me/{Config.BOT_USERNAME}?start=streaamdb_{str_to_b64(str(SaveMessage.id))})"
        short_link = get_short(share_link)
        await editable.edit(
            f"**𝘉𝘢𝘵𝘤𝘩 𝘍𝘪𝘭𝘦𝘴 𝘚𝘵𝘰𝘳𝘦𝘥 𝘪𝘯 𝘮𝘺 𝘋𝘢𝘵𝘢𝘣𝘢𝘴𝘦!**\n\n𝙃𝙚𝙧𝙚 𝙞𝙨 𝙩𝙝𝙚 𝙋𝙚𝙧𝙢𝙖𝙣𝙚𝙣𝙩 𝙇𝙞𝙣𝙠 𝙤𝙛 𝙮𝙤𝙪𝙧 𝙛𝙞𝙡𝙚𝙨: {short_link} \n\n"
            f"𝘑𝘶𝘴𝘵 𝘊𝘭𝘪𝘤𝘬 𝘵𝘩𝘦 𝘭𝘪𝘯𝘬 𝘵𝘰 𝘨𝘦𝘵 𝘺𝘰𝘶𝘳 𝘧𝘪𝘭𝘦𝘴!",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("⚡️ File Link ⚡️", url=short_link)],
                 [InlineKeyboardButton("🔊How To Open Link", url="https://t.me/streaamdb/1578")]]
            ),
            disable_web_page_preview=True
        )
        await bot.send_message(
        ...
        if(Config.LAZY_MODE == True):
            ...
            file_name = re.sub(r"@\w+|(_|\-|\.|\+)", " ", str(message.caption))
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("ミ★ GΞΓ FILΞ ★彡", url=short_link)],
                 [InlineKeyboardButton("🔊How To Open Link", url="https://t.me/streaamdb/1578")],
                ]
            )
